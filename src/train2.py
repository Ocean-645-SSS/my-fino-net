...
# ========== 4. 数据加载器 ==========
# 修改前：tf_list = [transforms.ToTensor()]
tf_list = [
    transforms.ToTensor(),
...
class DatasetFD(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        for i in frame_idx:
            ...
            img_batch.append(torch.cat([img, depth], dim=0)) # 多模态融合
...
class FinoNetRGBD(nn.Module):
    def __init__(self):
        ...
        self.convlstm4 = ConvLSTM(256, [256], (3, 3), 1, batch_first=True)
    ...
    def forward(self, x):
        ...
        x = x.view(b, t, 256, x.shape[-2], x.shape[-1])
...
class FinonetRGBDA(nn.Module):
    ...
    def forward(self, x, x_audio):
        ...
        # todo:加入加权融合
        # 原来：feat = torch.cat([vis, aud], dim=1)
        vis = self.w_vis * vis
        aud = self.w_aud * aud
        feat = torch.cat([vis, aud], dim=1)
        self.w_vis = nn.Parameter(torch.tensor(0.5))
        self.w_aud = nn.Parameter(torch.tensor(0.5))
