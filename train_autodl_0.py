...
for action in action_list:
    ...
    for bag in bags:
        ...
        fail_timestamp = 0
        """
        if bag.startswith('.') or not bag.isdigit():
            continue
        matching_rows = annotation_df[annotation_df["name"] == int(bag)]            
        # 2. 安全检查：如果这个文件夹在 annotation.txt 里没写标签，就跳过
        if matching_rows.empty:
            print(f"  跳过: 文件夹 {bag} 在 annotation.txt 中未找到标注")
            continue        
        row = matching_rows.iloc[0]
        bag_label = int(row["label"])       
        fail_timestamp = 0.0
